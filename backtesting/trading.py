...
SEQ_LEN = 60
path = "/Users/seonghyunpark/Documents/autotrading/backtesting/crypto_data/Binance_BTCUSDT_minute.csv"
...
def backtesting(close_value, close_time, budget):
    ...
    START_TIME = 100
    MINUTE = 5
    valueQ = deque((close_value[:START_TIME:MINUTE])[:16])
    trade_count = 0
    ...
    # looping 
    #change
    strat = RCstrategy(valueQ, budget, 2)
    
    for i in range(0, len(close_value)):
        
        price = close_value[i]
        if track_history.trade_count == 670:
            stop = False
        ...
        if traded:
            ...
            track_history.printCurrent(price, strat.coin, strat.deposit, close_time[i], strat.lastPrice, strat.trade_status)
